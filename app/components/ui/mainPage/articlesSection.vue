  ...
  <section class="py-24">
    <div class="container mx-auto px-4">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 ">
        <!-- مقالات کوچک -->
        <div class="lg:col-span-8 grid grid-cols-1 sm:grid-cols-2 grid-rows-none sm:grid-rows-2 gap-6">
          <NuxtLink
        ...
        >
          ...
          <div class="absolute inset-0 bg-black/30 flex flex-col justify-end p-6">
            <h3 class="text-xl sm:text-2xl lg:text-3xl font-bold text-white">{{ articles[0]?.title }}</h3>
            <p class="text-sm sm:text-base text-white mt-2 line-clamp-4">{{ articles[0]?.description }}</p>
          </div>
        </NuxtLink>
      </div>
    </div>
