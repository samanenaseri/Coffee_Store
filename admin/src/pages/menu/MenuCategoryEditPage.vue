  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-bold text-gray-800">
        {{ isEdit ? 'ویرایش دسته‌بندی منو' : 'افزودن دسته‌بندی منو' }}
      </h1>
    </div>
      ...
      <form @submit.prevent="save" class="space-y-6 max-w-3xl">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <ImageUploader v-model="form.icon" folder="menu-categories" />
          </div>
          <div>
...
async function fetchCategory() {
  try {
    const res = await api.get(`/admin/menu-categories/${route.params.id}`)
    ...
    toast.add({ severity: 'error', summary: 'خطا', detail: 'بارگذاری دسته‌بندی منو انجام نشد', life: 3000 })
...
async function save() {
  ...
  try {
    if (isEdit.value) {
      await api.put(`/admin/menu-categories/${route.params.id}`, form.value)
      toast.add({ severity: 'success', summary: 'موفق', detail: 'دسته‌بندی منو بروزرسانی شد', life: 3000 })
    } else {
      await api.post('/admin/menu-categories', form.value)
      toast.add({ severity: 'success', summary: 'موفق', detail: 'دسته‌بندی منو ایجاد شد', life: 3000 })
    }
    router.push('/menu-categories')
  } catch (e: any) {
