  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-bold text-gray-800">
        {{ isEdit ? 'ویرایش تصویر' : 'افزودن تصویر' }}
      </h1>
    </div>
      ...
      <form @submit.prevent="save" class="space-y-6 max-w-3xl">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <ImageUploader v-model="form.image" folder="gallery" />
          </div>
          <div>
...
async function fetchItem() {
  try {
    const res = await api.get(`/admin/gallery/${route.params.id}`)
    const item = res.data
    form.value = {
      title: item.title,
      slug: item.slug,
    ...
    toast.add({ severity: 'error', summary: 'خطا', detail: 'بارگذاری تصویر انجام نشد', life: 3000 })
    router.push('/gallery')
  }
}
...
async function save() {
  ...
  try {
    if (isEdit.value) {
      await api.put(`/admin/gallery/${route.params.id}`, form.value)
      toast.add({ severity: 'success', summary: 'موفق', detail: 'تصویر بروزرسانی شد', life: 3000 })
    } else {
      await api.post('/admin/gallery', form.value)
      toast.add({ severity: 'success', summary: 'موفق', detail: 'تصویر ایجاد شد', life: 3000 })
    }
    router.push('/gallery')
  } catch (e: any) {
